				...
				Embeds: []*discordgo.MessageEmbed{
					{
						...
						Footer: &discordgo.MessageEmbedFooter{
							Text: "This is a one-time acknowledgment per interaction session.",
						},
					},
				},
...
func (b *TetoBot) OnMessageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if mentioned || isDM {
		...
		if !agreed {
			b.Session.ChannelMessageSendReply(m.ChannelID, "Note: By chatting with Teto, you agree to Discord's ToS and the Bot Owner's ToS. (This notification is shown only once).", m.Reference())
			_ = b.Memory.RecordToSAgreement(userID)
		}
